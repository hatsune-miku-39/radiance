            ...
            var radiance_pack = {
                ...
                characterSort: {
                    "mode_extension_星耀璨然": {
                        ...
                        "radiance_free": [
                            "radiance_fr001_lotustar", "radiance_fr002_lizhaoyue", "radiance_fr004_veronia",
                            "radiance_fr006_yunzhongzi", "radiance_fr015_xiuyin",
                ...
                dynamicTranslate: {
                    ...
                    "radiance_renlin": function(player) {
                        if (player.storage.radiance_renlin) {
                            return "游戏开始时，你可以随机使用一张武器牌。其他角色的武器牌进入弃牌堆后，你可以获得之。<span class='legendtext'>限定技，当一名角色进入濒死状态时，你可以重铸一张武器牌，然后该角色回复X点体力并摸X张牌。（X为此牌的攻击范围）</span>";
                        }
            ...
            skill: {
                skill: {
                            ...
                            effect: {
                                target: function(card, player, target, current) {
                                    if (get.tag(card, 'damage') && get.attitude(player, target) < 0) {
                                        ...
                                        if (get.nature(card) && get.nature(card) != 'none') return 1.3;
                    ...
                    "radiance_dizui": {
                        ...
                        content: function() {
                            ...
                            player.chooseTarget(1, '选择【涤罪】的目标', function(card, player, target) {
                                return target != player && player.inRange(target) <= 1 && player.getHistory('custom', function(evt) {
                    ...
                    "radiance_qiongguang": {
                        ...
                        content: function() {
                            ...
                            if (result.moved[2].length > 0 && event.target.isIn()) {
                                event.target.gain(result.moved[2], 'gain2');
                    ...
                    "radiance_tianji": {
                        ...
                        content: function() {
                            'step 0'
                            player.chooseBool("天玑：失去一点体力来取消" + get.translation(trigger.card) + "？").set('ai', function() {
                                var trigger = _status.event.getTrigger();
                                var player = _status.event.player;
                ...
                translate: {
                    ...
                    "radiance_dianlu": "典录",
                    "radiance_dianlu_info": "每回合限X次，当与你距离1或以内的角色使用或打出一张【闪】后，你可以摸一张牌。你与其他角色计算距离时-X。（X为你已损失的体力值+1）",
                    "radiance_dizui": "涤罪",
                    ...
                    "radiance_tianji": "天玑",
                    "radiance_tianji_info": "当你成为一张其他角色的牌的目标时，你可以弃置一张牌并失去一点体力，然后取消此目标，且此牌对你无效。当你失去一点体力后，可以选择比7大或小。你持续展示并获得牌堆顶的牌，直至出现符合条件的牌。",
                    ...
                    "radiance_baiyi": "百艺",
                    "radiance_baiyi_info": "出牌阶段限一次，你可以弃置任意张手牌并令一名其他角色选择一项：1.弃置两倍的手牌；2.受到一点伤害，然后你摸等量的牌。",
                    "radiance_fengxing": "风行",
                    "radiance_fengxing_info": "当你成为一张【杀】的目标时，你可以失去一点体力取消之，然后你亮出牌堆顶的两张牌并令此牌的使用者选择一项：1.弃置一张点数在这两张牌的点数所构成的闭区间之内的手牌并令你获得这两张牌；2.你回复一点体力。",
