            ...
            skill: {
                skill: {
                    ...
                    "radiance_yingling": {
                        ...
                        filter: function(event, player, name) {
                            if (player.countCards('he') < player.hp || event.num <= 0 || event.source == event.player) return false;
                            if (name == 'damageBegin1') return event.player.hp > player.hp;
                            if (!event.source || !event.source.isIn() || event.source.hp > player.hp) return true;
                            return false;
                        },
                        direct: true,
                        content: function() {
                            'step 0'
                            var num = Math.max(player.hp, 1);
                            var dialog = ui.create.dialog("英灵");
                            var str = event.triggername == 'damageBegin4' ? "是否弃置" + get.cnNumber(num) + "张牌来取消" + (trigger.source ? get.translation(trigger.source) + "对你的" : "此") + "伤害？" : "是否弃置" + get.cnNumber(num) + "张牌对" + get.translation(trigger.player) + "额外造成一点伤害？";
                            dialog.addText(str);
                            player.chooseToDiscard(dialog, 'he', num).set('ai', function(card) {
                                var trigger = _status.event.getTrigger();
                                var player = _status.event.player;
                                if (player == trigger.source && get.damageEffect(trigger.player, player, player) < 1) return 0;
                        ...
                        subSkill: {
                            damage: {
                                ...
                                filter: function(event, player) {
                                    return player.hp > player.countMark('radiance_yingling') && trigger.player.hp > player.hp && event.num > 0;
                                },
                                logTarget: 'player',
                                check: function(event, player) {
                                    if (event.player.getEquip('baiyin') && !player.hasSkillTag('unequip') && !event.player.hasSkillTag('unequip2')) return false;
                                    return get.damageEffect(event.player, player, player) > 0;
                                },
                                prompt: "令此伤害+1",
                                content: function() {
                                    'step 0'
                                    player.addMark('radiance_yingling', 1, false);
                                    'step 1'
                                    trigger.num++;
                                },
                            },
                            clear: {
                                ...
                                forced: true,
                                content: function() {
                                    player.storage.radiance_yingling = 0;
                                    player.unmarkSkill('radiance_yingling');
                                },
                            },
                        },
                        ai: {
                    ...
                    "radiance_tianshu": {
                        ...
                        content: function() {
                            ...
                            player.draw(num);
                            if (num > trigger.num) trigger.cancel();
                        },
                    },
                    "radiance_huiwang": {
                        trigger: {
                        ...
                        filter: function(event, player) {
                            ...
                            var num = 0;
                            if (player.storage.radiance_huiwang && player.storage.radiance_huiwang.used) num = player.storage.radiance_huiwang.used;
                            if (num + 1 <= target.storage.radiance_huiwang.count) {
                ...
                translate: {
                    ...
                    "radiance_yingling_info": "当你受到伤害时，若此伤害没有来源或伤害来源体力值大于你，你可以弃置X张牌取消此伤害。你对体力值大于你的角色造成伤害时，可以弃置X张牌令此伤害+1。（X为你的体力值且至少为1）",
                    "radiance_tianshu": "天枢",
                    "radiance_tianshu_info": "锁定技，你的手牌上限至少为X。摸牌阶段开始时，你将手牌摸至X张，然后若以此法摸牌数大于此阶段的额定摸牌数，你跳过此阶段。（X为场上最高的体力值且至少为2，至多为5）",
                    "radiance_huiwang": "辉望",
                    "radiance_huiwang_info": "每回合每名角色限一次，当一名角色一回合内第X或更多次成为其以外的角色的伤害或失去牌效果的牌的目标后，你可以摸一张牌，然后交给其一张牌。（X为你本回合已发动此技能的次数+1）",
                    ...
                    "radiance_tianquan_info": "你的回合结束时，你可以回复一点体力或摸两张牌，然后你可以恢复一个“极光”已用牌名。",
                    "radiance_jimie": "迹灭",
