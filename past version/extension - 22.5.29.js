        ...
        content: function(config, pack) {
            ...
            var radiance_pack = {
                character: {
                    ...
                    "radiance_ma006_raimund": ["male", "radiance_group_magic", 3, ["radiance_xianggan", "radiance_zhihuan"],
                        []
                    ],
                ...
                characterSort: {
                    "mode_extension_星耀璨然": {
                        "radiance_magic": [
                            ...
                            "radiance_ma006_raimund", "radiance_ma007_tima", "radiance_ma009_draco",
                ...
                characterTitle: {
                    ...
                    "radiance_ma006_raimund": "空与物的变换者",
            ...
            skill: {
                skill: {
                    ...
                    "raidance_shenyi": {
                        ...
                        filter: function(event, player) {
                            return game.hasPlayer(function(target) {
                                return target.hp != Math.max(1, player.hp);
                            });
                        },
                        direct: true,
                        content: function() {
                            ...
                            if (result.bool) {
                                ...
                                var difference = Math.max(1, player.hp) - target.hp;
                                if (target.maxHp < player.hp && difference > 0) {
                                    difference = target.maxHp - player.hp;
                                };
                    ...
                    "raidance_shenyi_af": {
                        ...
                        intro: {
                            mark: function(dialog, content, player) {
                                if (!player.storage.raidance_shenyi_af) return "神意无效果";
                                if (player.storage.raidance_shenyi_af > 0) return "回合结束时回复" + player.storage.raidance_shenyi_af + "点体力并随机弃置等量的牌";
                                if (player.storage.raidance_shenyi_af < 0) return "回合结束时失去" + (0 - player.storage.raidance_shenyi_af) + "点体力并摸等量的牌";
                            },
                        },
                        trigger: {
                            player: ["phaseEnd"],
                        ...
                        filter: function(event, player) {
                            if (!player.storage.raidance_shenyi_af) return false;
                            return true;
                        },
                        content: function() {
                            'step 0'
                    ...
                    "radiance_yekong": {
                        ...
                        content: function() {
                            ...
                            player.draw(3);
                            'step 2'
                            var next = player.chooseToMove("夜空：将至少两张牌分别置于牌堆顶、牌堆底或弃置");
                ...
                translate: {
                    ...
                    "radiance_tianquan_info": "你的回合结束时，你可以摸一张牌，且若你体力值为全场最低，改为回复1点体力或摸两张牌；然后你可以恢复一个“极光”已用牌名。",
                    "radiance_jimie": "迹灭",
                    ...
                    "radiance_wenyi": "瘟疫",
                    "radiance_wenyi_info": "一名其他角色回复体力后，你记录其，然后可以令其弃置一张牌；你的结束阶段开始时，你可以对一名记录过的角色造成1点伤害，然后清空所有记录。",
                    "radiance_yujia": "驭假",
                    ...
                    "raidance_shenyi": "神意",
                    "raidance_shenyi_info": "回合开始时，你可以选择一名体力值与你不同的角色，令该角色将体力值变为与你相同（不视为伤害/失去/回复体力）。该角色的回合结束时，其回复以此法减少的体力值并随机弃置等量的牌/其失去以此法增加的体力值（保留至少一点体力）并摸实际失去体力数值的牌。",
                    "raidance_shenyi_af": "神意",
                    "radiance_jishi": "纪世",
                    "radiance_jishi_info": "每轮限一次，当一名角色的濒死结算结束后，你可以于当前回合结束后进行一个额外的回合。",
                    "radiance_mengmei": "梦魅",
