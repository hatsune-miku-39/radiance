        ...
        content: function(config, pack) {
            ...
            var radiance_pack = {
                ...
                characterSort: {
                    "mode_extension_星耀璨然": {
                        ...
                        "radiance_heng": [
                            ...
                            "radiance_he016_liyexin", "radiance_he017_qilian",
                        ],
                        "radiance_union": [
                ...
                translate: {
                    ...
                    "radiance_un017_qiuguxun": "秋谷薰",
                    "radiance_un018_kinsey": "金茜",
        ...
        package: {
            ...
            skill: {
                skill: {
                    ...
                    "radiance_kaiyang": {
                        ...
                        filter: function(event, player) {
                            ...
                            return red.length > Math.max(0, black.length);
                        },
                        multiline: true,
                        line: 'fire',
                        logTarget: 'player',
                    ...
                    "radiance_wuying": {
                        ...
                        content: function() {
                            ...
                            trigger.player.chooseControl(["选项一", "选项二"]).set('ai', function(event, player) {
                                ...
                                if (player.hp - event.player.hp < 2 || target.countCards('h') >= 2) return 0;
                                return 1;
                            }).set('choiceList', [
                    ...
                    "radiance_duanju": {
                        ...
                        content: function() {
                            ...
                            var next = game.createEvent('radiance_duanju_damage');
                            next.player = player;
                            ...
                            next.setContent(function() {
                                if (player.isIn() && source.isIn() && source.getGainableCards(player, 'he').length > 0) {
                                    var last = player.getHistory('damage', function(evt) {
                                        return evt.num > 0 && evt.source == source && evt.card == card && evt.getParent('useCard') == trigger;
                                    });
                                    if (last.length > 0) player.gainPlayerCard(source, num, true);
                                };
                            });
                            _status.event.next.remove(next);
                            trigger.after.push(next);
                        },
                        // group: ['radiance_duanju_damage', 'radiance_duanju_clear'],
                        subSkill: {
                            damage: {
                                trigger: {
                                    player: 'damageEnd'
                                ...
                                filter: function(event, player) {
                                    ...
                                    if (!player.storage.radiance_duanju.contains(event.card)) return false;
                            ...
                            clear: {
                                ...
                                filter: function(event, player) {
                                    if (!event.card) return false;
                                    return (player.storage.radiance_duanju.contains(event.card));
                                },
                                content: function() {
                                    'step 0'
                                    player.storage.radiance_duanju.remove(trigger.card);
                                },
                            },
                        },
                        action_tag: {
                    ...
                    "radiance_chuilian": {
                        ...
                        content: function() {
                            ...
                            if (trigger.player != player && player.isDamaged() && !player.hasSkill('radiance_chuilian_used')) {
                                player.recover();
                                player.addTempSkill('radiance_chuilian_used');
                            };
                        },
                    ...
                    "radiance_longjuan": {
                        ...
                        content: function() {
                            ...
                            player.chooseTarget(1, event.bool, event.bool ? "选择一名角色将其一张牌移动给你" : "是否再移动一张牌？", function(card, player, target) {
                                ...
                                var player = _status.event.player;
                                var es = target.getEquip('e', function(cardx) {
                                    return player.canEquip(cardx);
                                });
                                var max = -1;
                                for (var i = 0; i < es.length; i++) {
                    ...
                    "radiance_jishi": {
                        ...
                        content: function() {
                            ...
                            if (trigger.player.hasSkill('raidance_shenyi_af')) player.draw(2);
                        },
                        subSkill: {
                            used: {
                                charlotte: true,
                ...
                translate: {
                    ...
                    "radiance_aofa_db": "奥法",
                    "radiance_jingyao": "晶耀",
                    "radiance_jingyao_info": "限定技，出牌阶段，你可以将至多两张牌置于武将牌上。你的准备阶段开始时，移除你武将牌上的一张牌。只要你武将牌上有“晶耀”牌，你“奥法”的摸牌数+1。",
                    ...
                    "radiance_zhihuan": "置换",
                    "radiance_zhihuan_info": "当你成为一张其他角色的牌的目标时，你可以失去1点体力，然后取消此目标；当你失去1点体力或受到1点属性伤害后，可以亮出牌堆顶的X张牌。对于其中的每种花色，你可以执行以下一项：1.移动场上一张花色相同的牌；2.弃置一名角色区域内的一张牌。（X为触发此技能时你已损失的体力值）",
                    "radiance_lingyao": "灵药",
                    ...
                    "radiance_shouyue": "授钺",
                    "radiance_shouyue_info": "出牌阶段限一次，你可以将一张【杀】或武器牌交给其他角色，然后你摸三张牌，本回合手牌上限+2。",
                    ...
                    "radiance_kaiyang": "开阳",
                    "radiance_kaiyang_info": "其他角色的回合结束时，若本回合有进入弃牌堆的红色牌且数量大于黑色牌数，你可以对其造成1点火焰伤害。",
                    "radiance_wuying": "舞英",
                    "radiance_wuying_info": "你的回合外，当你每回合第一次成为其他角色牌的目标后，你令其选择一项：1.你获得X张♥牌；2.你本回合可以将任意一张手牌当做【杀】或【闪】使用或打出。（X为其体力值减去你的体力值且至少为1至多为3）",
                    "radiance_duanju": "断拒",
                    ...
                    "radiance_yingmu": "鹰目",
                    "radiance_yingmu_info": "出牌阶段限一次，你摸X张牌。你的手牌上限+X。（X为你的攻击范围，至少为1，至多为5）",
                    "radiance_lunxin": "论心",
                    ...
                    "radiance_huoluan": "碧瑶",
                    "radiance_huoluan_info": "出牌阶段限一次，你可以将一名角色装备区内的一张牌移动至另一名角色的装备区，若获得装备的角色不是你，失去了装备牌的角色视为对获得了牌的角色使用了一张无视防具的【杀】。",
                    "radiance_zhemian": "遮面",
                    ...
                    "radiance_xianyi": "仙衣",
                    "radiance_xianyi_info": "锁定技，当你的体力值变化后，若这是本局游戏中你的体力值第一次变为该值，你摸两张牌并回复1点体力。",
                    "radiance_quhai": "去害",
                    "radiance_quhai_info": "出牌阶段，你可以选择一名装备区内有武器的角色，令其选择一项：1.弃置装备区内的武器牌并摸X张牌；2.失去1点体力，然后此技能本阶段失效。（X为你本回合此前已发动“去害”的次数，至多为4）",
                    "radiance_heyu": "鹤羽",
                    ...
                    "radiance_huoshi": "祸世",
                    "radiance_huoshi_info": "出牌阶段限一次，你可以选择所有体力值不大于你或不小于你的角色，你与这些角色各失去1点体力。",
                    "radiance_jiguang": "极光",
                    ...
                    "radiance_jiaoyan": "娇艳",
                    "radiance_jiaoyan_info": "出牌阶段，若你本阶段没有造成过伤害，你可以选择一名其他角色，交给其一张伤害牌，然后其选择一项：1.对另一名你选择的角色使用一张【杀】（无距离限制，伤害来源视为你），然后你可以观看并获得其一张非伤害牌；2.将手牌补至四张并翻面。结束阶段，若你本回合使用“娇艳”的次数大于1，你失去1点体力。",
                    "radiance_tianhua": "天华",
                    ...
                    "radiance_xiangyun_info": "当你使用或打出红桃牌时，或当你成为其他角色红桃牌的目标后，可以摸一张牌；你的红桃牌不计入手牌上限。",
                    "radiance_suping": "肃平",
                    ...
                    "radiance_shiren": "失刃",
                    "radiance_shiren_info": "出牌阶段限一次，你可以对一名其他角色造成一点伤害。若如此做，直至你的下一个回合开始，你的所有【闪】均视为【杀】，你使用【杀】无距离限制。",
                    ...
                    "raidance_shenyi": "神意",
                    "raidance_shenyi_info": "回合开始时，你可以选择一项：1.令一名体力值不为全场唯一最高的角色将体力值变为1，其下个回合结束时回复X点体力；2.令一名角色将体力值回复至上限，其下个回合结束时失去X点体力，且保留至少1点体力。（X为以此法变化的体力值数，此变化不视为伤害或失去/回复体力）你对受到“神意”效果影响的角色使用牌无距离限制。",
                    "raidance_shenyi_af": "神意",
                    "radiance_jishi": "纪世",
                    "radiance_jishi_info": "每轮限一次，当一名角色的濒死结算结束后，你可以于当前回合结束后进行一个额外的回合。若该角色当前受“神意”效果影响，你额外摸两张牌。",
                    "radiance_mengmei": "梦魅",
