        ...
        content: function(config, pack) {
            ...
            var radiance_pack = {
                character: {
                    ...
                    "radiance_001_aides": ["female", "radiance_group_free", 3, ["raidance_shenyi", "radiance_jishi"],
                        ['forbidai']
                    ],
                ...
                characterSort: {
                    "mode_extension_星耀璨然": {
                        ...
                        "radiance_heng": [
                            "radiance_he001_xueli", "radiance_he003_yuchizhongming", "radiance_he005_zhaoshaolong",
            ...
            skill: {
                skill: {
                    ...
                    "raidance_shenyi": {
                        ...
                        content: function() {
                            ...
                            player.chooseTarget(1, get.prompt2('raidance_shenyi'), function(card, player, target) {
                                return (!target.isMaxHp(true) && target.hp > 1) || target.isDamaged();
                            }).set('ai', function(target) {
                                var player = _status.event.player;
                                var current = player.storage.radiance_jishi || player;
                            ...
                            if (result.bool) {
                                ...
                                if (target.isMaxHp(true)) {
                                    ...
                                    player.chooseControl("变为1血", "变为满血").set('ai', function(event, player) {
                                        if (get.attitude(player, target) > 1) return 0;
                                        return 1;
                            ...
                            var difference = 0;
                            if (choice == 0) {
                                difference = target.hp - 1;
                                target.hp = 1;
                            } else if (choice == 1) {
                                difference = target.hp - target.maxHp;
                    ...
                    "radiance_jishi": {
                        ...
                        filter: function(event, player, name) {
                            return !player.hasSkill('radiance_jishi_used');
                        },
                        content: function() {
                            player.addTempSkill('radiance_jishi_used', 'roundStart');
                            var next = player.insertPhase('radiance_jishi', false);
                            player.storage.radiance_jishi = _status.currentPhase;
                            var next2 = game.createEvent('radiance_jishi_clear', false, next);
                            next2.player = player;
                            next2.setContent(function() {
                                delete player.storage.radiance_jishi;
                            });
                            player.draw(2);
                        },
                        subSkill: {
                ...
                translate: {
                    ...
                    "radiance_guiji": "轨迹",
                    "radiance_guiji_info": "当一名角色使用牌指定你为目标时，你可以令此牌对你无效并令一名其他角色代替你成为此牌的目标（可以选择非法角色）；若此牌不为伤害牌或目标数大于1，你摸两张牌。最后你失去1点体力。",
                    "radiance_aofa": "奥法",
                    ...
                    "raidance_shenyi": "神意",
                    "raidance_shenyi_info": "回合开始时，你可以选择一项：1.令一名体力值不为全场唯一最高的角色将体力值变为1，其下个回合结束时回复X点体力；2.令一名角色将体力值变为与体力上限相同，其下个回合结束时失去X点体力，且保留至少1点体力。（X为以此法变化的体力值数，此变化不视为伤害或失去/回复体力）你对受到“神意”效果影响的角色使用牌无距离限制。",
                    "raidance_shenyi_af": "神意",
                    "radiance_jishi": "纪世",
                    "radiance_jishi_info": "每轮限一次，当一名角色的濒死结算结束前，你可以摸两张牌，然后于当前回合结束后进行一个额外的回合。",
                    "radiance_mengmei": "梦魅",
