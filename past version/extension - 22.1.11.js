                ...
                dynamicTranslate: {
                    ...
                    "radiance_renlin": function(player) {
                        if (player.storage.radiance_renlin) {
                            return "游戏开始时，你可以随机使用一张武器牌。其他角色的武器牌进入弃牌堆后，你可以获得之。你的武器牌不计入手牌上限。<div style='opacity:0.5'>限定技，当一名角色进入濒死状态时，你可以重铸一张武器牌，然后该角色回复等量的体力。</div>";
                        }
            ...
            skill: {
                skill: {
                    ...
                    "radiance_yingling": {
                        ...
                        content: function() {
                            'step 0'
                            player.chooseToDiscard('英灵：是否弃置' + get.cnNumber(player.hp) + '张牌来取消此伤害？', 'he', player.hp).set('ai', function(card) {
                        ...
                        group: 'radiance_yingling_damage',
                    ...
                    "radiance_huiwang": {
                        ...
                        filter: function(event, player) {
                            ...
                            var num = player.getStat('skill').radiance_huiwang;
                            if (!num) num = 0;
                ...
                translate: {
                    ...
                    "radiance_yingling_info": "当你受到伤害时，若此伤害没有来源或伤害来源体力值大于你，你可以弃置X张牌防止之。每轮限X次，你对体力值大于你的角色造成伤害时，可以令此伤害+1。（X为你的体力值）",
                    "radiance_tianshu": "天枢",
                    "radiance_tianshu_info": "锁定技，你的手牌上限+X。摸牌阶段开始时，你改为将手牌补至手牌上限，然后若以此法摸了至少X张牌，你跳过摸牌阶段。（X为场上最高的体力值且至少为2，至多为5）",
                    "radiance_huiwang": "辉望",
                    "radiance_huiwang_info": "每回合每名角色限一次，当一名角色一回合内第X或更多次成为其他角色的伤害或失去牌效果的牌的目标后，你可以摸一张牌，然后交给其一张牌。（X为你本回合已发动此技能的次数+1）",
